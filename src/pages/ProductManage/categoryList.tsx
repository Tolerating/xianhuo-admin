...
import {  pageCategoryRequire,
  updateCategoryStatus,
  addCategoryStatus } from '@/api/product';
import { PageContainer } from '@ant-design/pro-components';

const initTable = async (params: any) => {
  console.log(params);
  const { current, pageSize} = params;
  let result = await pageCategoryRequire(current, pageSize);
  return {
...
const Post: React.FC = () => {
  ...
  const onCheck = async () => {
    try {
      ...
      const result = await addCategoryStatus(values.name)
      ...
      render: (row, record) => (
        <>
          {Number(row.status) === 1 && (
            <Popconfirm
              title="提示"
              description="确定将该分类置为无效？"
              onConfirm={() => {
                updateCategoryStatus(row.id, 0).then((res) => {
                  console.log(res);
                  messageApi.open({
                    type: 'success',
                    content: res.message,
