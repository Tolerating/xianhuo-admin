...
import {pageProductComplain,dealProductComplain,complainProductById} from '@/api/complain'
...
const initTable = async (params:any)=>{
  ...
  let result = await pageProductComplain(current,pageSize,startTime,endTime);
  return {
...
const columns:ProColumnsType = [
...

  {
    ...
    render: (row, record) => (
     <>
      ...
      onConfirm={()=>{
        dealProductComplain(row.id,row.complainantSubject,row.complainantId,row.sellerId,0).then(res=>{
          console.log(res);
          messageApi.open({
            type: 'success',
            content: res.message,
  ...
  <Popconfirm
      ...
      onConfirm={()=>{
        dealProductComplain(row.id,row.complainantSubject,row.complainantId,row.sellerId,1).then(res=>{
          messageApi.open({
            type: 'success',
            content: res.message,
          });
    ...
    itemLayout="horizontal">
      ...
      <List.Item>
        <List.Item.Meta
          avatar={<EnvironmentOutlined />}
          title="商品状态"
          description={<Tag>{Number(complainInfo.status)===-1?"已下架":StatusMap[complainInfo.status]}</Tag>}
        />
      </List.Item>
      <List.Item>
        <List.Item.Meta
